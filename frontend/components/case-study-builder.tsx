...
const CaseStudyBuilder = ({
  ...
  return (
    <div className="bg-gray-50 h-full flex flex-col">
      <nav className="h-[68px] flex items-center justify-between px-3 bg-white border-b border-gray-200 shadow-[0px_2px_5px_0px_rgba(0,0,0,.04)] shrink-0">
          ...
          <div className="inline-flex items-center gap-x-3">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    onClick={handleSave}
                  >
                    <Clock className="size-[15px]" /> {lastSavedAt ? `Saved at ${lastSavedAt}` : "Save"}
                  </Button>
                </TooltipTrigger>
                <TooltipContent sideOffset={9} visual="gray" side="bottom">
                  {lastSavedAt ? `Last saved at ${lastSavedAt}` : "Click to save"}
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <IconButton
              className={cn(
                ...
                isPreview && "bg-primary-500 text-white"
              )}
              type="button"
              onClick={() => setIsPreview(!isPreview)}
            >
              {isPreview ? <EyeOff className="size-3" /> : <Eye className="size-3" />}
            </IconButton>

            <Button type="button" onClick={handlePublish}>Publish</Button>
          </div>

          <IconButton className="text-gray-500" variant="ghost" visual="gray" type="button" onClick={onClose}>
            <X className="size-6" />
          </IconButton>
        </div>
      </nav>
