...
async function run() {
  ...
  console.log("👉 Please login manually (LinkedIn)...")
  console.log(
    "👉 After successful login, the browser will automatically save the authenticated state to tests/.auth/linkedin.json"
  )
  // 👇 You login manually here

  /*  skip manual login entirely:
  // click LinkedIn button manually OR automate selector
  ...
  */
  ...
  // IMPORTANT: wait for token to be set
  await page.waitForTimeout(3000)

  // ✅ Save session state to file for use in tests
  await context.storageState({ path: "tests/.auth/linkedin.json" })

  console.log("Saved LinkedIn login state!")
...
// AFTER running, you can delete the "playwright-user-data" folder that was created, as it's not needed for the tests. The important output is the "tests/.auth/linkedin.json" file which contains the authenticated state for your tests.
...
// globalSetup: './tests/setup/linkedin.setup.ts', to the projects or test.use in config.ts to use the saved login state in tests.
