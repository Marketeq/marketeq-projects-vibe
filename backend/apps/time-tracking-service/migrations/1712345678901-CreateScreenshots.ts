...
export class CreateScreenshots1712345678901 implements MigrationInterface {
  ...
  public async up(qr: QueryRunner): Promise<void> {
    ...
    `);
    await qr.query(`CREATE UNIQUE INDEX IF NOT EXISTS ux_screenshots_key ON public.screenshots ("key")`);
    await qr.query(`CREATE INDEX IF NOT EXISTS idx_screenshots_user_created ON public.screenshots (user_id, created_at DESC)`);
    await qr.query(`CREATE INDEX IF NOT EXISTS idx_screenshots_group_key ON public.screenshots (group_key) WHERE group_key IS NOT NULL`);
    await qr.query(`CREATE INDEX IF NOT EXISTS idx_screenshots_visible ON public.screenshots (user_id, created_at DESC) WHERE is_deleted = false`);
  }
  ...
  public async down(qr: QueryRunner): Promise<void> {
    ...
    await qr.query(`DROP TABLE IF EXISTS public.screenshots`);
  }
}
